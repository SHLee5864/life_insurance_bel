...
disc as (
    select cohort_id, sex, scenario_id, version_id, mort_version_id, discount_ok from {{ ref('val_discounting_sanity') }}
),
bel as (
    select cohort_id, sex, scenario_id, version_id, mort_version_id, bel_reconciled from {{ ref('val_bel_reconciliation') }}
),
sens as (
    select cohort_id, sex, scenario_id, version_id, mort_version_id, sensitivity_ok from {{ ref('val_sensitivity_direction') }}
)
...
left join bel b on p.cohort_id = b.cohort_id and p.sex = b.sex and p.scenario_id = b.scenario_id and d.version_id = b.version_id and d.mort_version_id = b.mort_version_id
left join sens s on p.cohort_id = s.cohort_id and p.sex = s.sex and p.scenario_id = s.scenario_id and d.version_id = s.version_id and d.mort_version_id = s.mort_version_id 