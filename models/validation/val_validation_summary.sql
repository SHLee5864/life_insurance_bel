...
disc as (
    select cohort_id, sex, scenario_id, discount_ok from {{ ref('val_discounting_sanity') }}
),
bel as (
    select cohort_id, sex, scenario_id, bel_reconciled from {{ ref('val_bel_reconciliation') }}
),
sens as (
    select cohort_id, sex, scenario_id, sensitivity_ok from {{ ref('val_sensitivity_direction') }}
)
...
left join bel b on p.cohort_id = b.cohort_id and p.sex = b.sex and p.scenario_id = b.scenario_id
left join sens s on p.cohort_id = s.cohort_id and p.sex = s.sex and p.scenario_id = s.scenario_id